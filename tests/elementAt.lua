...
describe('elementAt', function()
  it('errors when its parent errors', function()
    expect(Observable:throw():elementAt(0)).to.produce.error()
  end)

  it('chains subscriptions', function()
    local subSpy = spy()
    local observable = Observable.create(function() subSpy() end)
  ...
  it('errors if no index is specified', function()
    expect(function () Observable.of(1):elementAt() end).to.fail()
  end)

  it('produces no values if the specified index is less than one', function()
    expect(Observable.of(1):elementAt(0)).to.produce.nothing()
    expect(Observable.of(1):elementAt(-1)).to.produce.nothing()
  end)

  it('produces no values if the specified index is greater than the number of elements produced by the source', function()
    expect(Observable.of(1):elementAt(2)).to.produce.nothing()
  end)
